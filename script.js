$(function(){
    ...
    ], autoSlide = setInterval(autoChange, 3000);
    ...
    function autoChange(){
        let numCurr = curr;
        curr = curr + 1 <= 2 ? curr + 1 : 0;
        changeImg(numCurr, curr);
    }

    function changeImg(numCurr, numNext){
        slidesImage.eq(numCurr).hide()
        $('.detail-image-text').html(arr[numNext].title)
        $('.detail-image-date').html(arr[numNext].date)
        counts.html(numNext + 1)
        slidesImage.eq(numNext).fadeIn()
    }
